...
const egyptianTheme: Theme = {
  ...
  styles: {
    ...
    // Golden glow shadows
    shadowCard:
      '0 4px 20px -2px rgba(197, 160, 40, 0.15), 0 2px 8px -2px rgba(0, 0, 0, 0.1)',
    shadowElevated:
      '0 8px 30px -4px rgba(197, 160, 40, 0.25), 0 4px 12px -2px rgba(0, 0, 0, 0.15)',
    ...
    bgOverlay:
      'linear-gradient(180deg, transparent 0%, rgba(197, 160, 40, 0.02) 50%, rgba(197, 160, 40, 0.04) 100%)',
...
const kaibaCorpTheme: Theme = {
  ...
  styles: {
    ...
    shadowCard:
      '0 4px 20px -2px rgba(0, 100, 200, 0.2), 0 2px 8px -2px rgba(0, 0, 0, 0.15)',
    shadowElevated:
      '0 8px 30px -4px rgba(0, 150, 255, 0.3), 0 4px 12px -2px rgba(0, 0, 0, 0.2)',
    ...
    bgOverlay:
      'linear-gradient(135deg, rgba(0, 100, 200, 0.02) 0%, transparent 50%, rgba(0, 150, 255, 0.03) 100%)',
...
export function useThemeSystem() {
  ...
  const currentTheme = computed(
    () => THEMES[currentThemeId.value] || THEMES.egyptian
  )
  const colors = computed(() =>
    isDark.value ? currentTheme.value.dark : currentTheme.value.light
  )
  ...
  function setTheme(themeId: string): void {
    if (THEMES[themeId]) {
      ...
      // reload page to fully reset theme-related styles
