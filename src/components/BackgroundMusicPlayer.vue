<template>
  <div class="fixed bottom-4 right-4 z-50">
    ...
    <div 
      v-show="!isMinimized"
      class="bg-themed-card border border-themed rounded-xl shadow-xl w-100 overflow-hidden"
    >
      <!-- Header -->
      ...
      <div class="max-h-48 overflow-y-auto">
        <button
          ...
          class="w-full px-4 py-2 text-left hover:bg-themed-secondary/50 transition-colors flex items-center gap-3"
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-themed truncate">{{ track.title }}</p>
            <p class="text-xs text-themed-muted truncate">{{ track.artist }}</p>
          </div>
        </button>
      </div>
